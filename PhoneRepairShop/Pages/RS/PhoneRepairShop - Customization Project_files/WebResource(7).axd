...
var Editor = (function(){
  ...
  function Editor(options)
  {
    ...
    if (!options.readOnly) {
      ...
      addEventHandler(document.body, "paste", function(event) {
      	if (!self.fire("paste", event)) return;
      	cursorActivity();
        var text = null;
        try {
  ...
  Editor.prototype = {
  	subscribe: function(event, handler)
  	{
  		...
  		if (this.subscribers[event].indexOf(handler) == -1) this.subscribers[event].push(handler);
  	},
  	unsubscribe: function(event, handler)
  	{
  		if (!this.subscribers[event]) throw new Error("Now such event " + event + " to unsubscribe from");
  		this.subscribers[event] = this.subscribers[event].filter(function (a) { return a != handler });
  	},
  	fire: function(event, ev){
  		if (!this.subscribers[event]) throw new Error("Now such event " + event + " to fire");
  		if (this.subscribers[event].some(function(a){return false === a(ev);})) return false;
  		return true;
  	},
    ...
    keyDown: function (event) {
    	if (!this.fire("keydown", event)) return;
      ...
      function highlight(node, ok) {
        ...
        if (self.options.markParen) {
          ...
          node.style.fontWeight = "bold";
            node.style.color = ok ? "#F92472" : "#F88";
        }
      }
